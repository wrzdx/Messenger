...
func TestGetUsers(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			name:  "all users",
			users: test_utils.MockUsers,
		},
		{
			name:  "limit users",
			users: test_utils.MockUsers[:1],
			limit: new(1),
		},
		{
			name:      "negative limit",
		...
		},
		{
			...
			users:  test_utils.MockUsers[1:2],
			limit:  new(1),
			offset: new(1),
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tt.users, gotUsers, cmpopts.EquateApproxTime(time.Microsecond)); diff != "" {
