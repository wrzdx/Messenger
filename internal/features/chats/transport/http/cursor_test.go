...
func TestChatCursor(t *testing.T) {
	t.Run("encodes nil cursor as nil", func(t *testing.T) {
		encoded, err := encodeChatCursor(nil)
	...
	t.Run("round trips cursor", func(t *testing.T) {
		expected := &chats_service.ChatCursor{
			ChatID:         uuid.MustParse("00000000-0000-0000-0000-000000000001"),
			LastActivityAt: time.Date(2026, time.July, 19, 12, 30, 0, 0, time.UTC),
		}

		encoded, err := encodeChatCursor(expected)
		require.NoError(t, err)
		require.NotNil(t, encoded)

		actual, err := decodeChatCursor(*encoded)
	...
	testCases := []struct {
		...
		},
		{
			...
			cursor: base64.URLEncoding.EncodeToString([]byte(`{"chat_id":`)),
		},
		{
			name: "rejects nil chat id",
			cursor: base64.URLEncoding.EncodeToString([]byte(
				`{"last_activity_at":"2026-07-19T12:30:00Z"}`,
			)),
		},
		{
			name: "rejects zero last_activity_at",
			cursor: base64.URLEncoding.EncodeToString([]byte(
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			cursor, err := decodeChatCursor(testCase.cursor)
	...
	t.Run("does not classify encoding failure as invalid request", func(t *testing.T) {
		_, err := encodeChatCursor(&chats_service.ChatCursor{
			ChatID:         uuid.New(),
			LastActivityAt: time.Date(10000, time.January, 1, 0, 0, 0, 0, time.UTC),
		})
